...
## Fucntion performing the NPNT Pre-Arm Checks
def PreArm_Checks(Permission_Artifact):
        Date_check = False
        Time_check = False
        geo_check = False
        coord_list, Date_check, Time_check = parseXML(Permission_Artifact) # Performing the Date and Time Check
        if Date_check == True and Time_check == True:
            if verifySig(Permission_Artifact, Cert) == True:
                print("Signature Verification Done. Performing Geo-fence check now...")
                len_file = len(coord_list)

                my_fence = picket.Fence()
                if(len_file>0):
                    for i in range(len_file):
                        my_fence.add_point((float(coord_list[i][0]), float(coord_list[i][1])))
                    Lat, Lon, _ = lat_long_alt()
                ...
                if(geo_check == True):
                    ...
                    print("PRE_ARM CHECKS FAILED. DRONE COULD NOT BE ARMED.")
            else:
                print("Signature could not be verified.DRONE COULD NOT BE ARMED.")
        else:
            print("Date and Time CHECKS FAILED. DRONE COULD NOT BE ARMED.")
...
if __name__ == '__main__':
    ...
    parser.add_option("--map", action='store_true', help=" map module")
    parser.add_option(
        '--load-module',
        action='append',
        default=[],
    ...
    #print("..................")
    #print(get_distance_metres(28.31423, 77.16247))
    #print(mavutil.location.lat)
    
    #test_in_fence_real_latlong_large_scale()
